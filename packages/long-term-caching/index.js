...
const outputName = (dev, chunk = false) => dev ? (
...
const createCachingGroups = (cachingGroups) => {
    ...
    if (Array.isArray(cachingGroups)) {
        ...
        Object.keys(cachingGroups).forEach((key) => {
            let test = null;
            let moduleCheck = cachingGroups[key];
...
module.exports = function razzleLongTermCaching(baseConfig, env, webpack, userOptions = {}) {
    ...
    if (!(cachingOptions === true || cachingOptions === false || splitWithSize)) {
        getCacheGroup = createCachingGroups(cachingOptions);
    }

    let { vendorsChunkName, sizeOptions } = options;
    ...
    if (cachingOptions && !splitWithSize) {
        vendorsName = function (module) {
            ...
            const hashedName = crypto.createHash('md5').update(packageName).digest('hex');
